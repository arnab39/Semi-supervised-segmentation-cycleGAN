...
class semisuper_cycleGAN(object):
    ...
    def train(self, args):
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            ### For displaying the images generated by generator on tensorboard
            with torch.no_grad():
                fake_label = self.Gsi(l_img).detach()
                fake_unlabel = self.Gsi(unl_img).detach()
            fake_prediction_label = fake_label.data.max(1)[1].squeeze_(1).squeeze_(0).cpu().numpy()
            fake_prediction_unlabel = fake_unlabel.data.max(1)[1].squeeze_(1).squeeze_(0).cpu().numpy()
            ### display_tensor is the final tensor that will be displayed on tensorboard
            display_tensor_label = torch.zeros([fake_label.shape[0], 3, fake_label.shape[2], fake_label.shape[3]])
            display_tensor_unlabel = torch.zeros([fake_unlabel.shape[0], 3, fake_unlabel.shape[2], fake_unlabel.shape[3]])
            for i in range(fake_prediction_label.shape[0]):
                ...
                new_img_unlabel = fake_prediction_unlabel[i]
                new_img_unlabel = utils.colorize_mask(new_img_unlabel)   ### So this is the generated image in PIL.Image format
