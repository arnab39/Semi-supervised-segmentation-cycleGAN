...
class supervised_model(object):
    def __init__(self, args):
        ...
        # Define the network 
        self.Gsi = define_Gen(input_nc=3, output_nc=22, ngf=args.ngf, netG='resnet_9blocks_softmax', norm=args.norm,
    ...
    def train(self, args):
        ...
        if self.args.dataset == 'voc2012':
            labeled_set = VOCDataset(root_path=root, name='label', ratio=1.0, transformation=transform,
                                     augmentation=None)
            val_set = VOCDataset(root_path=root, name='val', ratio=1.0, transformation=transform,
                                 augmentation=None)
...
class semisuper_cycleGAN(object):
    def __init__(self, args):
        ...
        self.Gsi = define_Gen(input_nc=3, output_nc=22, ngf=args.ngf, netG='resnet_9blocks_softmax',
                              norm=args.norm, use_dropout=not args.no_dropout, gpu_ids=args.gpu_ids)
    ...
    def train(self, args):
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, ((l_img, l_gt, _), (unl_img, _, _)) in enumerate(zip(labeled_loader, unlabeled_loader)):
                ...
                fake_gt = self.Gsi(unl_img.float())  ### having 22 channels
                lab_gt = self.Gsi(l_img)  ### having 22 channels
                ...
                unsupervisedloss = args.adversarial_weight*(img_gen_loss + gt_gen_loss) + (img_cycle_loss + gt_cycle_loss)*args.lamda
